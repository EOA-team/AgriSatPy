...
from radiu.snap_rut.s2_rut_algo import S2RutAlgo
import numpy as np
import datetime
import os
import xml.etree.cElementTree as ET  # C implementation is much faster and consumes significantly less memory
from radiu.snap_rut.s2_l1_rad_conf import u_diff_temp_rate
...
class S2RutOp:
    def __init__(self):
        ...
        self.rut_algo = S2RutAlgo()
        self.unc_band = None
        self.toa_band = None
        # S2A launch date 23-june-2015 and S2A launch date 7-march-2017, time is indifferent.
        self.time_init = {'Sentinel-2A': datetime.datetime(2015, 6, 23, 10, 0),
    ...
    def get_u_diff_temp(self, datastrip_meta, band_id):
        ...
        return (time_start - self.time_init[self.spacecraft]).days / 365.25 * \
               u_diff_temp_rate[self.spacecraft][band_id]
